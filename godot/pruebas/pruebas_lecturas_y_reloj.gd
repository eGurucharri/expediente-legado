...
static func lecturas(archivo, comprobar: Callable) -> void:
	...
	comprobar.call("releer cerrado sin acciones permite leer", archivo.registro_actual["folio"], folio)
	...
	archivo._al_elegir_documento(1)
	comprobar.call("repetir la lectura gratuita no duplica el folio", archivo.jornada["leido_hoy"], [folio])

	var guardada := Partida.new()
	comprobar.call("la lectura gratuita se guarda", guardada.cargar()["resultado"], "cargada")
	comprobar.call("recargar conserva la lectura gratuita", guardada.estado["jornada"]["leido_hoy"], [folio])
	comprobar.call("releer conserva la firma", guardada.estado["veredictos"][caso_id], firma)
	comprobar.call("releer no descubre pistas", archivo.descubiertas, pistas)
	var fuentes := SuenoContenido.fuentes(
		archivo.jornada["leido_hoy"], archivo.contenido.casos, pistas,
	...
	comprobar.call("sin acciones no se abre un caso nuevo", archivo.registro_actual.is_empty(), true)
	comprobar.call("una apertura denegada no se recuerda", archivo.jornada["leido_hoy"].has(otro), false)
	...
	comprobar.call("solo se registran lecturas realizadas", archivo.jornada["leido_hoy"], [folio, otro])
...
static func reloj(comprobar: Callable) -> void:
	...
	comprobar.call("el reloj refleja el tiempo consumido", is_equal_approx(Jornada.noche_restante(noche), 0.6), true)
	noche["sueno_escenas"].pop_front()
	comprobar.call("atravesar una salida no rejuvenece la noche", is_equal_approx(Jornada.noche_restante(noche), 0.6), true)
	Jornada.gastar_sueno(noche, total * 0.1)
	comprobar.call("el reloj sigue bajando tras la salida", is_equal_approx(Jornada.noche_restante(noche), 0.5), true)
	...
	comprobar.call("recargar conserva la proporción nocturna", is_equal_approx(Jornada.noche_restante(cargada), 0.5), true)
	comprobar.call("recargar conserva las salas pendientes", cargada["sueno_escenas"], noche["sueno_escenas"])
	comprobar.call("recargar conserva la duración total", cargada.get("sueno_total", 0.0), total)
	...
	comprobar.call("se puede cargar al agotarse el reloj", releida.cargar(ruta)["resultado"], "cargada")
...
static func _migracion(comprobar: Callable) -> void:
	...
	comprobar.call("migrar fija una referencia conservadora", antigua.get("sueno_total", 0.0), referencia)
	var proporcion := Jornada.noche_restante(antigua)
	antigua["sueno_escenas"].pop_front()
	Jornada.completar(antigua)
	comprobar.call("completar otra vez no recalcula el total", antigua.get("sueno_total", 0.0), referencia)
	comprobar.call("una noche migrada tampoco retrocede", is_equal_approx(Jornada.noche_restante(antigua), proporcion), true)
	...
	comprobar.call("despertar de golpe también limpia el total", sin_reloj.get("sueno_total", -1.0), 0.0)
